  ...
  const xhsApp = {
    ...
    async mount(container, roche) {
      ...
      async function generateFeed(){
        ...
        const prompt=`你現在要為小紅書生成 ${count} 篇貼文。這些貼文不是「${name}」自己發的，而是「${name}」在刷小紅書時會看到的、各種路人用戶發的貼文。

${ctx?`以下是「${name}」的個性和近期狀況，用來決定推薦算法會推給TA什麼樣的內容：\n${ctx}`:''}

要求：
- 每篇貼文由不同的路人帳號發出（請隨機生成有趣的暱稱）
- 內容要多樣化：美食探店、穿搭分享、生活吐槽、旅行打卡、職場碎念、深夜emo、搞笑日常、寵物、健身、攝影、時事吐槽等
- 如果有角色資料，部分貼文可以巧妙踩中角色的興趣點或近期經歷（比如角色最近去了某個地方、喜歡某種食物、從事某種工作），讓人感覺「推薦算法真的懂我」
- 風格要像真正的小紅書：標題吸睛（可以用emoji、誇張語氣），正文有分段、有emoji、有口語化表達
- 每篇都要有 author（發帖人暱稱）、title、content（150-300字）、tags（3-5個）、coverEmoji、date（如「2小時前」「昨天」「3天前」）
...
直接回覆 JSON 陣列，不要有任何其他文字。`;
  ...
  window.RochePlugin.register({
    id:'roche-xiaohongshu',name:'小紅書',version:'4.0.0',
